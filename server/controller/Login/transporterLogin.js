import User from '../../models/user.js'; // Renommez correctement ici le modèle
...
router.post('/login', async (req, res) => {
    try {
        ...
        // Recherche du transporteur associé à l'utilisateur
        const transporter = await User.findOne({ _id: user._id });
        if (!transporter) {
            console.log('Aucun transporteur trouvé pour l\'utilisateur:', user._id);
            return res.status(401).json({ error: 'Aucun transporteur associé à cet utilisateur.' });
        }
        ...
        const accessToken = jwt.sign(
            { userId: user._id, role: user.role, transporterId: transporter._id }, // Utiliser transporter._id ici
