...
function authenticateToken(req, res, next) {
    try {
        ...
        jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
            ...
            // 6. Ajouter les informations de l'utilisateur dans la requête pour les étapes suivantes
            req.user = user;
            console.log(`Accès autorisé : ${user.username || user.email} (ID: ${user.userId})`);
            next();  // Continuer avec la requête
        });

    } catch (error) {
        // 7. Si une erreur se produit dans le processus, retourner une erreur serveur interne
